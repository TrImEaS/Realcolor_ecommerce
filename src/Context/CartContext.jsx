...
export const CartProvider = ({ children }) => {
  ...
  const addProductToCart = ({ product, quantity_selected = 1 }) => {
    ...
    if (productExistInCart) {
      if ((productExistInCart.quantity_selected + quantity_selected) > product.stock) { return Swal.fire('Atención', 'No hay más stock disponible para agregar al carrito', 'info') }
    ...
    if (newProduct.quantity_selected > newProduct.stock) { return Swal.fire('Atencion', 'No hay mas stock disponible para agregar al carrito', 'info') }
  ...
  return (
    <CartContext.Provider value={{ cartProducts, completeOrder, addProductToCart, deleteOneProductOfCart, deleteProductOfCart, getTotalOfProducts, cleanCart }}>
      {children}
