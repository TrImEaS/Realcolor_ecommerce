  ...
  return (
    <div className={`fixed top-0 z-[999999] right-0 w-[80%] max-sm:w-full min-h-[100%] cart flex flex-col max-w-[500px] overflow-y-auto gap-5 p-3 bg-gray-100 border-2 text-gray-800 ${showCart === 3 && 'hidden'} ${showCart ? 'show' : 'hide'}`}>
      <button onClick={()=> setShowCart(0)}>
        <FaTimesCircle className="text-xl text-blue-500"/>
      </button>
        ...
        {getTotalOfProducts() === 0 
          ...
          : (
            <article className="flex flex-col gap-3 w-full justify-center border-b-2 pb-4 border-gray-200 items-center">
              <h3 className="font-bold border-b-2 border-blue-500 px-3 tracking-wider text-blue-500">
                Mi carrito
              </h3>
              ...
              <div className="flex justify-around items-center w-full px-5">
                <NavLink to='/cart' onClick={()=> setShowCart(0)} className='group flex flex-col'>
                  <p className="flex items-center gap-x-1 text-blue-500">
                    <FaDotCircle className="text-sm"/> 
                    <span>Ir a mi carrito</span>
                  </p>
                  <span className="group-hover:w-full h-[1.5px] w-0 bg-blue-500 duration-300 text-blue-500"></span>
                </NavLink>
                ...
                <button onClick={() => cleanCart()} className="group flex flex-col">
                  ...
                  <span className="group-hover:w-full h-[1.5px] w-0 bg-blue-500 duration-300"></span>
                </button>
              </div>
            </article>
          )
      ...
      <section className="px-5 overflow-y-auto h-[26rem] flex flex-col gap-5">
        {getTotalOfProducts() > 0 &&
          cartProducts.map(product => (
            <article key={product.id} className="flex items-center max-h-[180px] p-3 gap-5 w-full shadow-lg rounded-xl bg-white text-gray-800 hover:shadow-xl transition-all duration-300 border border-transparent hover:border-blue-100">
              ...
              <aside className="w-[60%] min-h-[100px] flex flex-col text-xs gap-y-2 justify-between">
                <NavLink onClick={()=> setShowCart(0)} className='flex flex-col hover:text-blue-700 transition-colors duration-300'  to={`/products?product=${product.sku}`}>
                ...
                <div className="w-full flex justify-center text-white items-center mt-1">
                  <button onClick={()=> deleteOneProductOfCart({ productID: product.id })} className="w-8 h-7 bg-gradient-to-r from-blue-500 to-blue-600 rounded-l-md flex justify-center items-center border-0 hover:from-blue-600 hover:to-blue-700 transition-all duration-300 font-bold">-</button>
                  <span className="min-w-8 min-h-7 bg-blue-500 flex justify-center items-center border-x-0 px-2 font-medium">{product.quantity_selected}</span>
                  <button onClick={()=> addProductToCart({ product })} className="w-8 h-7 bg-gradient-to-r from-blue-600 to-blue-500 rounded-r-md flex justify-center items-center border-0 hover:from-blue-700 hover:to-blue-600 transition-all duration-300 font-bold">+</button>
                </div>
              </aside>
            </article>
          ))
        ...
        {getTotalOfProducts() > 0 &&
          <NavLink
            ...
            className="bg-gradient-to-r from-blue-500 to-blue-600 fixed bottom-5 w-[90%] z-[10000] hover:from-blue-600 hover:to-blue-700 hover:scale-[1.02] text-white py-4 px-6 rounded-lg text-center font-bold tracking-wide transition-all duration-300 flex items-center justify-center gap-3 shadow-md"
          >
